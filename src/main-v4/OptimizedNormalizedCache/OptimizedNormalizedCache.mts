 ...
 * ```
 */
export default class OptimizedNormalizedCache extends ApolloCache {
  ...
  >(query: Cache.DiffOptions<TData, TVariables>): DataProxy.DiffResult<TData> {
    ...
    if (query.returnPartialData !== false) {
      ...
      if (missing.length > 0) {
        return {
          complete: false,
          result: proxy as DeepPartial<TData> | null,
  ...
  public override restore(serializedState: unknown): this {
    ...
    for (const key in serializedState) {
      ...
      d[key] = {
        __proto__: null,
        ...(serializedState as Record<string, object>)[key],
      };
    }
    for (const key in serializedState) {
      const targetParent =
      ...
      storeToObject(
        ...
        key,
        (serializedState as Record<string, unknown>)[key]
      );
    }
    this.data = d;
    return this;
  ...
  public override extract(_optimistic?: boolean): unknown {
    const rootStore = this.data;
    return JSON.parse(JSON.stringify(rootStore)) as unknown;
  }
  ...
  public override fragmentMatches(
    ...
    const typenameToUse = fragment.typeCondition.name.value;
    if (typename === typenameToUse) {
      return true;
    }
    if (!this.supertypeMap) {
      return false;
    }
    return (
      getActualTypename(typename, typenameToUse, this.supertypeMap) != null
    );
  }
